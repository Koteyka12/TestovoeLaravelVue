...
const props = defineProps({
    activePage: {
        type: String,
        default: 'settings' // 'reviews' или 'settings'
    },
    organization: {
        type: Object,
        default: null
...
const page = usePage();
const userName = page.props.auth?.user?.name || 'Название аккаунта';
</script>

<template>
    <aside class="w-[280px] min-h-screen bg-[#F6F8FA] flex flex-col">
        <!-- Header area with logo -->
        <div class="h-[75px] flex items-center px-7">
            <img src="/images/logo.png" alt="Daily Grow" class="h-7 w-[140px] object-contain object-left" />
        </div>
        
        <!-- Account name -->
        <div class="px-7 pb-4">
            <span class="text-base font-bold text-[#6C757D] tracking-[0.2px] leading-5" style="font-family: 'Mulish', sans-serif;">{{ userName }}</span>
        </div>

        <!-- Navigation -->
        <nav class="pt-3 mx-[16px] border-t border-[#DFE0EB]">
            <!-- Заголовок раздела с белым фоном -->
            <div class="h-[48px] flex items-center pl-[14px] rounded-[12px] bg-white shadow-[0px_2px_1px_rgba(0,0,0,0.02)]">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 21 20" fill="none" class="mr-[12px]">
                ...
                <span class="text-[16px] font-medium leading-5 text-[#363740]" style="font-family: 'Mulish', sans-serif;">Отзывы</span>
            </div>
            ...
            <div class="mt-3 space-y-1">
                <Link
                    ...
                    class="block py-1 text-xs text-[#363740]"
                    :class="[
                        activePage === 'reviews' 
                            ? 'px-[50px] bg-white rounded-[12px] shadow-[0px_2px_1px_rgba(0,0,0,0.02)]' 
                            : 'px-[50px] hover:text-[#363740]',
                        { 'opacity-50 pointer-events-none': !organization && activePage !== 'reviews' }
                    ]"
                ...
                <Link
                    ...
                    class="block py-1 text-xs"
                    :class="activePage === 'settings' 
                        ? 'px-[50px] text-[#363740] bg-white rounded-[12px] shadow-[0px_2px_1px_rgba(0,0,0,0.02)]' 
                        : 'px-[50px] text-[#A4A6B3] hover:text-[#363740]'"
                    style="font-family: 'Mulish', sans-serif;"
                >
                    Настройка
                </Link>
