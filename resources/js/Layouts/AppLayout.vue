<script setup>
import { Link } from '@inertiajs/vue3';
import Sidebar from '@/Components/Sidebar.vue';

defineProps({
    organization: Object,
    activePage: {
        type: String,
        default: 'settings'
    }
});
</script>

<template>
    <div class="min-h-screen bg-white flex">
        <!-- Left Sidebar -->
        <Sidebar :activePage="activePage" :organization="organization" />

        <!-- Main Content -->
        <main class="flex-1 flex flex-col">
            <!-- Header -->
            <header class="h-[75px] flex items-center justify-end px-6 border-b border-[#DCE4EA]">
                <Link
                    :href="route('logout')"
                    method="post"
                    as="button"
                    class="text-gray-400 hover:text-gray-600"
                    title="Выйти"
                >
                    <svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" viewBox="0 0 44 44" fill="none">
                        <path d="M15.2 29.7778H24V32H15.2C13.99 32 13 31 13 29.7778V14.2222C13 13 13.99 12 15.2 12H24V14.2222H15.2V29.7778Z" fill="#909AB4"/>
                        <path d="M26.1 17L25.113 18.0071L28.319 21.2857H21V22.7143H28.319L25.106 25.9929L26.1 27L31 22L26.1 17Z" fill="#909AB4" stroke="#909AB4"/>
                    </svg>
                </Link>
            </header>

            <!-- Content -->
            <div class="px-[35px] py-[18px]">
                <slot />
            </div>
        </main>
    </div>
</template>
